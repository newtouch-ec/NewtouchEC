...
class b2c_ctl_admin_member_messenger extends desktop_controller {
    ...
    function index(){
        ...
        foreach ($message as $mkey => $mvalue) {
            ...
            foreach ($action as $key => $value) {
                ...
                $app_id = $tmpl[0];
                $tmpl[0] = "view/admin";
                $html_dir = implode('/',$tmpl).'/'.$key;
                
                $obj_app_id = kernel::service('b2c_messenger_tpl_appid');
                if ($obj_app_id && method_exists($obj_app_id, 'get_app_id')){
                    $obj_app_id->get_app_id($key, $app_id);
                }
                //echo ROOT_DIR.'/app/'.$app_id.'/'.$html_dir.'.html'.'<pre>';
                if(file_exists( ROOT_DIR.'/app/'.$app_id.'/'.$html_dir.'.html')){
                    $action[$key][$mkey]=true;
                }else{
                    $action[$key][$mkey]=false;
                }
            }
        }
		...
		$user_id = $user->get_id();
		$this->pagedata['is_super']= $user->is_super();
    ...
    function setSmsSign(){
        ...
        $this->pagedata['sign_lot']=is_array($setSmsSign) ? $setSmsSign['sign_lot'] : 'prefix';
    ...
    function viewtmpl($action,$msg){
        ...
        $this->pagedata['sign_lot']=is_array($setSmsSign) ? $setSmsSign['sign_lot'] : 'pre';
