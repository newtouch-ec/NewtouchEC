...
class b2c_ctl_site_cart extends b2c_frontpage{
    ...
    private function update_cart(&$msg='',$type='all') {
        ...
        foreach($aCart as $row)
        {
             ...
             {	if(isset($aParams['modify_quantity'][$row['obj_ident']]['adjunct'])){
					$adjunctUpdate = true;
					$allowednum = array_shift($row['params']['adjunct'][0]['adjunct']);
				}
				$update_row = $row;
				break;
    ...
    {	
		...
		foreach($this->pagedata['aCart']['object']['goods'] as $k=>$v){
            ...
            //判断店铺是否有效
            $store_rs = $mdl_storemanager->checkStoreActive($v['store_id']);
            if($store_rs == false){
                $this->splash('failed', 'back', app::get('b2c')->_('存在无效店铺的商品！'));
            }
