...
class b2c_ctl_site_passport extends b2c_frontpage{
    ...
    function post_login($url=null){
        ...
        if($member_id)
        {
            ...
            //登录测试和最后登录时间
            $sdf['login_time']=$sdf['clogin_time'];
            $sdf['clogin_time']=time();
            $sdf['login_count']=intval($sdf['login_count'])+1;
...
//                setcookie('MOBILE_CODE_TIMER', time());
                ...
                echo '<span>短信发送失败，请重新获取验证码</span>';exit;
            }
        }else{
            echo '<span>' . $message . '</span>';exit;
        }
    }
