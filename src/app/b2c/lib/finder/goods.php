...
class b2c_finder_goods{
    ...
    function column_control($row){
        $returnValue = '<a href="index.php?app=b2c&ctl=admin_goods_editor&act=edit&p[0]='.$row['goods_id'].'&finder_id='.$_GET['_finder']['finder_id'].'"  target="blank">'.app::get('b2c')->_('查看').'</a>';
        ...
        if($aGoods && $aGoods[0]['spec_desc']){
            $returnValue .= ' <a href="index.php?app=b2c&ctl=admin_products&act=set_spec_index&nospec=0&goods_id='.$row['goods_id'].'"  target="blank">'.app::get('b2c')->_('查看货品').'</a>';
        }
