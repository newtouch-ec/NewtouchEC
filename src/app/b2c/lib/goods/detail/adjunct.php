...
class b2c_goods_detail_adjunct{
    ...
    function _get_goods($gid){
        ...
        $aGoods = $objGoods->getList('goods_id,adjunct,spec_desc,name,image_default_id,price,store,marketable,nostore_sell,store_id', array('goods_id'=>$gid),0,1);
        $aGoods = $aGoods[0];
        ...
        if(is_array($aGoods['adjunct']) && $aGoods['adjunct']){
            foreach($aGoods['adjunct'] as $key => $rows){    //loop group
                ...
                if(isset($arr['type_id'])){
                    ...
                    //增加店铺id 过滤条件。 以防配件会选取到其他店铺的商品
                    $arr['store_id'] = $aGoods['store_id'];
