    ...
    public function order_refund_finish(&$sdf, $status='succ', $from='Back', &$msg='')
    {
        ...
        if ($order_data['payed'] < 0)
        {
            if($sdf_order['score_u']){
                if($sdf_order['score_u']*$point_deductible_valu < (-$order_data['payed'])){
                    $msg = app::get('b2c')->_('订单退款金额超过支付金额，不能退款！');
                    return false;
                }else{
                    $order_data['payed'] = $objMath->number_minus(array(($sdf_order['payed']+$sdf_order['score_u']*$point_deductible_valu), $sdf['cur_money']));
                    $pay_dead = '1';
                }                
            }else{
                $msg = app::get('b2c')->_('订单退款金额超过支付金额，不能退款！');
