...
//            return $this->ioTitle['csv'][$filter['type_id']];
    ...
    function orderBy($id=null){
        $order=array(//更新单一维度排序时如果排序相同则按照评分排序。
           ...
           2=>array('label'=>app::get('b2c')->_('按发布时间 新->旧'),'sql'=>'sdb_b2c_goods.last_modify desc,avg_point desc'),
           3=>array('label'=>app::get('b2c')->_('按发布时间 旧->新'),'sql'=>'sdb_b2c_goods.last_modify ,avg_point desc'),
           4=> array('label'=>app::get('b2c')->_('按价格 从高到低'),'sql'=>'sdb_b2c_goods.price desc,avg_point desc'),
           5=>array('label'=>app::get('b2c')->_('按价格 从低到高'),'sql'=>'sdb_b2c_goods.price,avg_point desc'),
           6=>array('label'=>app::get('b2c')->_('访问周次数'),'sql'=>'sdb_b2c_goods.view_w_count desc,avg_point desc'),
           7=> array('label'=>app::get('b2c')->_('总访问次数'),'sql'=>'sdb_b2c_goods.view_count desc,avg_point desc'),
           8=>array('label'=>app::get('b2c')->_('周购买次数'),'sql'=>'sdb_b2c_goods.buy_w_count desc,avg_point desc'),
           9=> array('label'=>app::get('b2c')->_('总购买次数'),'sql'=>'sdb_b2c_goods.buy_count desc,avg_point desc'),
           10=> array('label'=>app::get('b2c')->_('评论次数'),'sql'=>'sdb_b2c_goods.comments_count desc,avg_point desc'),
           11=> array('label'=>app::get('b2c')->_('月购买次数'),'sql'=>'sdb_b2c_goods.buy_m_count desc,avg_point desc'),
        );
...
//            break;
    ...
    function getDlytype($aGoods,$area_id){
        ...
        if ($dlytype && is_array($dlytype))
        {
            ...
            $objRegions = app::get('ectools')->model('regions');
            $arrRegion = $objRegions->dump($postAreaId);
