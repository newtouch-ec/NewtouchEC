...
class b2c_mdl_member_messenger {
    ...
    function _send($sendMethod,$tmpl_name,$target,$data,$type,$title=null){
	   ...
	   /** 加签名 **/
		...
		/** 加签名 只有短信发送才可以加签名**/
        $sign='';
        if('b2c_messenger_sms'==get_class($sender)){
            $sign='【'.$setSmsSign['sign'].'】';
        }
		if($setSmsSign['sign_lot'] && $setSmsSign['sign_lot']=='prefix'){
    ...
    function actionSend($type,$data,$member_id=null){
        ...
        foreach($senders as $sender){
           ...
           if($sender){
                if(isset($data['sendmobilecodetype']) && ($data['sendmobilecodetype'] == 'userRegister')){
                    $target = $data['contact']['phone']['mobile'];
                }else if(isset($data['sendemailcodetype']) && ($data['sendemailcodetype'] == 'userRegister')){
                    $target = $data['email'];
                }else{
                    $target = $this->get_send_type(kernel::single($sender)->sdfpath, $data, $member_id, $tmpl_name);
                }
    ...
    function actions(){
        $actions = array(
            ...
            'account-verifyEmail'=>array('label'=>app::get('b2c')->_('会员邮箱验证'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;'.app::get('b2c')->_('链接').'&nbsp;<{$url}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),
           ...
           'orders-ship'=>array('label'=>app::get('b2c')->_('买家发货申请'),'level'=>9,'varmap'=>app::get('b2c')->_('登录名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;'.app::get('b2c')->_('订单号').'&nbsp;<{$order_id}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ...
            'storemanger-register'=>array('label'=>app::get('b2c')->_('供应商入驻申请成功'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0//前后台控制发送的表示0为后台1为前台issue_money
            
            ),

            'storemanger-approved'=>array('label'=>app::get('b2c')->_('供应商入驻审核通过'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;Issname&nbsp;<{$Issname}>&nbsp;&nbsp;&nbsp;&nbsp;Issmoney&nbsp;<{$issue_money}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),

            'storemanger-approvefailed'=>array('label'=>app::get('b2c')->_('供应商入驻审核失败'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;Remark&nbsp;<{$Remark}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),

            'storemanger-punish'=>array('label'=>app::get('b2c')->_('供应商违规处罚'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),

            'storemanger-issue'=>array('label'=>app::get('b2c')->_('供应商扣除保证金'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),

            'storemanger-renew'=>array('label'=>app::get('b2c')->_('供应商到期续约'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),
            
            'storemanger-expired'=>array('label'=>app::get('b2c')->_('供应商过期关闭'),'level'=>9,'varmap'=>app::get('b2c')->_('用户名').'&nbsp;<{$uname}>&nbsp;&nbsp;&nbsp;&nbsp;','isIndex'=>0
            ),
