...
function ecos_site_lib_theme_widget_save_all($widgets_id,$match,&$return,&$slots)
{
	if(!$widgets_id){
		return false;
	}else{
...
function ecos_wap_lib_theme_widget_widgets_config_theme($name,&$data,$theme)
{
	$data['crun'] = 'theme_widget_cfg_' . $name;
	$data['cfg'] = $data['dir'] . '/theme_widget_cfg_' . $name . '.php';
	$data['run'] = 'theme_widget_' . $name;
...
function ecos_site_lib_theme_widget_widgets_get_libs_notype($info,$val,&$widgetsLib=array())
{
	...
	if($info['usual']=='1'){
		$widgetsLib['usual'][]=array('sort'=>$info['order'],'description'=>$info['description'],'name'=>$val['name'], 'app'=>$val['app'],'theme'=>$val['theme'],'label'=>$info['name'],'folder'=>$info['type']);
	}
}
function ecos_wap_lib_theme_widget_widgets_get_libs_notype($info,$val,&$widgetsLib=array())
{
	if($info['catalog']){
		if(!$widgetsLib['list'][$info['catalog']]){
			$widgetsLib['list'][$info['catalog']]=$info['catalog'];
...
function ecos_site_lib_theme_widget_widgets_get_libs_type($info,$type,$val,&$widgetsLib=array())
{
	if($info['catalog']==$type){
		...
		$widgetsLib['list'][] = array('sort'=>$info['order'],'description'=>$info['description'],'name'=>$val['name'], 'app'=>$val['app'],'theme'=>$val['theme'],'label'=>$info['name'],'folder'=>$info['type']);
	}
	/*
	if($info['usual']=='1'){
...
function ecos_wap_lib_theme_widget_widgets_get_libs_type($info,$type,$val,&$widgetsLib=array())
{
	if($info['catalog']==$type){
		$order[]=$info['order']?$info['order']:0;
		$widgetsLib['list'][] = array('sort'=>$info['order'],'description'=>$info['description'],'name'=>$val['name'], 'app'=>$val['app'],'theme'=>$val['theme'],'label'=>$info['name'],'folder'=>$info['type']);
...
function ecos_wap_lib_theme_widget_prefix_content(&$content, $widgets_dir)
{
	$pattern = array(
		'/(\'|\")(images\/)/is',
		'/((?:background|src|href)\s*=\s*["|\'])(?:\.\/|\.\.\/)?(images\/.*?["|\'])/is',
