...
class business_ctl_admin_settlement extends desktop_controller
{
    ...
    function index($settlement_no='',$type='')
    {
		$this->finder('business_mdl_settlement', 
		   ...
		   'actions' => array(
				array(
					'label' => "生成结算报表",
					'href' => 'index.php?app=business&ctl=admin_settlement&act=auto_create_page',
					'target' => 'dialog::{title:\'' . app::get('business')->_('生成结算报表') . '\',width:350,height:110}'
				),
			),
		   'use_view_tab'=>true,
		   'force_view_tab'=>true,
    ...
    function auto_create_page(){
		...
		if(empty($balances)){
			...
			foreach($balances as $val){
				$real_store[] = $val['store_id'];
			}
		}

		$stores = array_intersect($stores,$real_store);

        $this->pagedata['store_ids'] = $store_ids;
        $this->pagedata['store_count'] = sizeof($stores);
        $this->pagedata['order_count'] = sizeof($balances);
	...
	public function settlement_balance(){
        $this->begin('index.php?app=business&ctl=admin_settlement&act=nobalance');
		/*$settlement_no = array();
		...
		$res = $obj->update(array('is_balance'=>'1'),array('settlement_no'=>$settlement_no));
