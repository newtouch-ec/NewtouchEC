...
class business_view_input{
  ...
  function input_vimage($params){
        ...
        $html = '<div class="image-input clearfix" style="width:'.$imageInputWidth.'px;" gid="'.$domid.'">';
            ...
            $html.= '<img  style="width:'.$params['width'].'px;height:'.$params['height'].'px;" src="'.$image_src.'" onload="$(this).zoomImg('.$params['width'].','.$params['height'].',function(mw,mh,v){this.setStyle(&quot;marginTop&quot;,(mh-v.height)/2)});"/>';
...
// print_r($number);exit;
        if($params['show_name'] && $params['show_name']=='true'){
         ...
         //   $member_info = kernel::single('b2c_frontpage')->get_current_member();
            ...
            if(!$number || !$store_id ){
                $list .= "<span style=\"cursor:pointer;float:left;height: 50px; width:188px;overflow:hidden;\">{$store_info['store_name']}</span>";
            }else{
        ...
        foreach($store_info['store_point'] as $item){
            ...
            if($item['avg_percent'] > 0) {
            	...
            	$list .= "<span class=\"sdc-high\">".abs($item['avg_percent'])."%</span></li>";
            }else {
