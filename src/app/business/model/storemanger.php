...
function check_acountid($account_id, &$message) {
   ...
   function save(&$data,$mustUpdate = null){
       ...
       if($data['zip']){
        ...
        //店员保存店铺设置时，取店铺的ID保存  Add by PanF  begin
        if($data['store_id']){
            $ary_store= $this->getList('account_id,shop_name',array('store_id'=>$data['store_id']));

            if($ary_store && $ary_store[0]['account_id'] !=$data['account_id']){
                   $data['account_id']= $ary_store[0]['account_id'];
                   $data['shop_name']= $ary_store[0]['shop_name'];
            }
        }
        //店员保存店铺设置时，取店铺的ID保存  Add by PanF  begin
     ...
     *检测店铺是否是有效店铺
     */
    function checkStoreActive($store_id){
        $row = $this->getRow('limit_store,status',array('store_id'=>$store_id));
        if($row['limit_store'] == '1'){
            $this->merror = '店铺被屏蔽';
            return false;
        }
