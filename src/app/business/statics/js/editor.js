...
var mceHandler = new Class({
    ...
    exec: function(cmd, arg) {
        if (!this.busy) {
			...
			case 'insertImage':
				 this.inc.win.focus();
				this.inc.doc.execCommand('insertImage',false,arg);break;
            case "insertHTML":
				//yds
...
//
//						if(window.curRange){
//							var r = window.curRange;
//						}else{
//							var r = this.getRange();
//						}
//							var temp = this.inc.doc.createElement('span');
//							temp.innerHTML = arg+'&nbsp;';
//							r.insertNode(temp);
...
//                    }
					...
					if (document.selection){
						if(window.curRange){
							if(typeof window.curRange.moveStart === 'function' || typeof window.curRange.moveStart === 'object'){
								var range = doc.selection.createRange();
								range.moveStart('character',window.bookmark);
							}
					    }else{
							var range = doc.selection.createRange();
					    }
...
//				range.collapse(false);
    ...
    dialog: function(action, options) {
        if (!this.inc) return;
//        this.inc.win.focus(); // yds 为了解决插入图片光标移动到第一行问题
		window.curRange = this.range;
        this.range = null;
...
//        this.exec.bind(this)('insertHTML', html);
			this.exec.bind(this)('insertImage', image_src);//yds
...
//         console.log(image_src);
	...
	createBookmark: function(name) {
		var doc = this.inc.doc;
        var startNode ='<span style="display:none;" name='+name+'></span>';
//		startNode.id = '_bookmark_start_';
//		startNode.appendChild(doc.createTextNode('\u200D'));

		if (document.selection){
			var range = doc.selection.createRange();
//			console.log(range);
//			range.moveStart('character',range.boundingLeft);
//			var start = range.startContainer;
//			start.appendChild(startNode);
			range.pasteHTML(startNode);
		}
    },
});
