var editor_style_num = new Date().getTime();
    ...
    style_init: function() {
        ...
        this.addEvent('click', function(e) {
            if (!this.$init) {
                ...
                var obj = this;
                //this.mce_handle.setOpacity(1);
                $ES('img', this.mce_handle).addEvent('click', function(e) {
					
					if(document.selection && e.target.title=='插入表格'){						
						window.curRange = this.getRange();
						this.inc.win.focus();
						this.createBookmark(editor_style_num);

						var dochtml = this.inc.doc.body.innerHTML.toLocaleUpperCase();
						dochtml = dochtml.replace('&NBSP;','');
						dochtml = dochtml.replace('</STRONG>','');
						dochtml = dochtml.replace('<STRONG>','');
						dochtml = dochtml.replace(/<font[^>]+>/ig,"");
						dochtml = dochtml.replace("</FONT>","");
//						console.log(dochtml);

						window.bookmark = dochtml.indexOf('<SPAN STYLE="DISPLAY: NONE;" NAME="'+editor_style_num+'"></SPAN>')!=-1?dochtml.indexOf('<SPAN STYLE="DISPLAY: NONE;" NAME="'+editor_style_num+'"></SPAN>'):dochtml.indexOf('<SPAN STYLE="DISPLAY: NONE" NAME="'+editor_style_num+'"></SPAN>');
						
//						console.log(this.inc.doc.body.innerHTML.toLocaleUpperCase());
//						console.log('<SPAN STYLE="DISPLAY: NONE;" NAME="'+editor_style_num+'"></SPAN>');
                ...
                Ex_Loader('picker', function() {
                    new GoogColorPicker($E('.fontColorPicker', this.el), {
                        onSelect: function(hex, rgb, e) {
                            if ((!!window.ActiveXObject || "ActiveXObject" in window) && this.range){//by yds	
