 ...
 class cellphone_ctl_admin_column extends desktop_controller{
     ...
     function edit(){
		...
		if($data['is_active']=='true'){	
			...
			$this->end(false,app::get('b2c')->_('不能编辑已开启的项'));
		}
	...
	//exit;
	 ...
	 public function get_data(){
      ...
      if(!$data['columntype_id']){
	     $this->end(false,app::get('b2c')->_('请选择栏目类型'));
	  }
	  if(!$data['goods_id']){
	  ...
	  if(!$data['start_time']||!$data['end_time']){
           $this->end(false,app::get('b2c')->_('请填写完整的时间段'));
       }
      ...
      if($data['end_time']<=$data['start_time']){
           $this->end(false,app::get('b2c')->_('开始时间要大于结束时间'));
       }
	  if(!$data['image_id']){
	       $this->end(false,app::get('b2c')->_('请上传图片'));   
	  }
