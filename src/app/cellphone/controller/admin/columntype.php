	...
	$this->finder('cellphone_mdl_columntype',
	 ...
	 //
      ...
      if(!$data['columntype_name']){
	     $this->end(false,app::get('b2c')->_('栏目类型不为空'));
	  }
	  if(!$data['columntype_createtime']){
	     $this->end(false,app::get('b2c')->_('录入时间不为空'));
	  }
	  if(!$data['css_type']){
	     $this->end(false,app::get('b2c')->_('请选择样式类型'));
	  }
	  if(!$data['d_order']){
	     $this->end(false,app::get('b2c')->_('排序不能为空'));
	  }
	  if($data['d_order']&&!is_numeric($data['d_order'])){
	     $this->end(false,app::get('b2c')->_('排序必须为数字'));
	  }
