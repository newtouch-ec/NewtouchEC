...
class cellphone_base_homepage_banner extends cellphone_cellphone{
   ...
   //获取banner轮播区域启用的内容
    ...
    /* 此处获得首页图片广告的列表数据(后台启用的)*/
	  ...
	  $picad = app::get('cellphone')->model('picad');
	  $curtime = time();
      $data = 
	  $picad->getList('associate_id,associate_type,image_id,d_order',array('is_active'=>'true','start_time|lthan'=>$curtime,'end_time|than'=>$curtime),($params['npage']-1)*$params['pageLimit'],$params['pageLimit'],'d_order ASC');
	  if($data){	
		 foreach($data as $key=>&$val){
		   $val['image_id']=$this->get_img_url($val['image_id'],$picSize);
	  ...
	  $this->send(true,$data,app::get('cellphone')->_('图片广告列表'));
	  }else{	  
	  $this->send(true,null,app::get('cellphone')->_('没有数据'));
	  }
	}
...
where a.article_id ='.$article_id;
	...
	public function getexchangerate()
	{
		...
		$ectools_mdl_currency = app::get('ectools')->model('currency');
		$result = $ectools_mdl_currency->getcur($curCode);
		if($result){
			$this->send(true,$result,app::get('cellphone')->_('货币汇率'));
		}else{
			$this->send(true,null,app::get('cellphone')->_('没有数据'));
		}
	}
