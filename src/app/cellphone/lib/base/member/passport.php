...
class cellphone_base_member_passport extends cellphone_cellphone{
    ...
    public function login(){
        ...
        //$params['password'] = trim($this->des_decode($params['password'],$this->des_private_key));
         //非中藏平台注册用户不可登录本平台  2014-2-12  PanF   begin
        if($uc = kernel::service("uc_user_login")){
            //非中藏平台注册用户 uc_user_login  return false
            if($userInfo = $uc->uc_user_login($params['uname'],$params['password'])){
                $rows = $userInfo;

            }else{

               $password_string = pam_encrypt::get_encrypted_password(trim($params['password']), 'member', array('login_name' => trim($params['uname'])));

                if(!$params['uname'] || !$password_string || ($params['password'] !== '0' && !$params['password'])){
        ...
        //非中藏平台注册用户不可登录本平台  2014-2-12  PanF   end
        ...
        if($rows[0]){
		    ...
		    $encryptedpassword = pam_encrypt::get_encrypted_password(trim($params['password']), pam_account::get_account_type('b2c'), $use_pass_data);

            $rows = app::get('pam')->model('account')->getList('*',
                                        array('account_id' =>$rows[0]['account_id'],
                                                'login_password'=>trim($encryptedpassword),
                                                'account_type' =>'member', 'disabled' => 'false'), 0, 1);
    ...
    public function signup(){
        ...
        //seller
        if(isset($params['seller']) && !empty($params['seller']) && $params['seller']=='seller'){
    ...
    */
    public function memberInfo(){
        ...
        $member['mobile'] = $mobile['contact']['phone']['mobile'];
    ...
    function saveinfo(){
        ...
        if (!$member) {
            ...
            $data['firstname'] = strtoupper($params['firstname']);
            $data['lastname'] = strtoupper($params['lastname']);
            ...
            foreach ($data as $key => $val) {
                ...
                if (empty($data[$key])) {
                    switch ($key) {
                        case 'email':
                            $this->send(false, null, app::get('b2c')->_('EMAIL不能为空'));
                            break;
                        case 'mobile':
                            $this->send(false, null, app::get('b2c')->_('手机号码不能为空'));
                            break;
                        case 'passport':
                            $this->send(false, null, app::get('b2c')->_('护照号码不能为空'));
                            break;
                        case 'firstname':
                            $this->send(false, null, app::get('b2c')->_('姓(拼音大写)不能为空'));
                            break;
                        case 'lastname':
                            $this->send(false, null, app::get('b2c')->_('名(拼音大写)不能为空)'));
                            break;
                        default:
                            break;
                    }
            ...
            if(!$this->checkuppercase($data['firstname'])){
                $this->send(false, null, app::get('b2c')->_('姓(拼音大写)必须是大写'));
            }
            if(!$this->checkuppercase($data['lastname'])){
                $this->send(false, null, app::get('b2c')->_('名(拼音大写)必须是大写'));
            }

            $arr['member_id'] = $data['member_id'];
            $arr['contact']['email'] = $data['email'];
            $arr['contact']['phone']['mobile'] = $data['mobile'];
            $arr['contact']['passport'] = $data['passport'];
            $arr['contact']['firstname'] = $data['firstname'];
            $arr['contact']['lastname'] = $data['lastname'];
            $mdl_members = app::get('b2c')->model('members');
            if($mdl_members->save($arr)){
