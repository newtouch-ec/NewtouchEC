...
class cellphone_base_misc_cart extends cellphone_cellphone
{
        ...
        if($this->ajax_update === true){
            foreach(kernel::servicelist('b2c_cart_object_apps') as $object) {
                ...
                if($aTemp){
                    ...
                    $aTemp['buy_price'] = intval($aTemp['buy_price']);
    ...
    */
    ...
    function checkout($isfast=0){
		...
		//进入结算页前的判断-----------------------------------------------------add by XU 2015/06/17
		//验证商品数量
		$check_Cart = $this->mCart->get_objects();
		//echo "<pre>";print_r($aCart);exit;
		$goods_list = array();
		if($check_Cart['object']['goods']){
			foreach($check_Cart['object']['goods'] as $key=>$val){
				//数量组件
			...
			foreach($goods_list as $g_id=>$quantity){
				...
				if((int)$maxbuy['max_buy'] > 0){
					if((int)$maxbuy['max_buy'] < (int)$quantity){
						$this->send(false, null, app::get('b2c')->_("对不起，您购买的商品".$maxbuy['name']."超过限购个数".$maxbuy['max_buy']."，请修改数量！"));exit;
					}
				}
			}
		}
        ...
        foreach($aCart['order_split'] as $store_id => $items){
            ...
            $aData['order_items'][$store_id]['store_slip_subtotal'] = intval($totle); //店铺总金额合计（含运费）
...
//add by Seven for 收货地址追加地区区分 start
				'region_type'=>$address['region_type'],
//add by Seven for 收货地址追加地区区分 end
            );
            $area_id = $area[2];
            $area = explode('/',$area[1]);
    ...
    function update_total($store_id=0,$no_coupon){
        ...
        if($store_id){
            ...
            $aData['store_count'] = array(
                ...
                'coupon_name' =>$aCart['coupon'][$store_id]['name'],//优惠劵名称 add bu cuiqiwen
