...
,avg_point,freight_bear,store,store_freeze,notify_num,score,score_setting,weight,unit,min_buy,package_scale,package_unit,package_use,nostore_sell,disabled,spec_desc,brief,adjunct";
		...
		//2015/06/16 by XU add----------------------
		$aGoods['max_buy'] = $aGoods_list[0]['max_buy'];
		//------------------------------------------
...
WHERE p.goods_id ={$gid}";
    ...
    /*  if(!$siteMember['member_id']){
        ...
        if($aGoods['act_type'] && $aGoods['act_type'] != 'normal' && $aGoods['act_type'] != 'package'){
            ...
            $app_id = $aGoods['act_type']=='group'?'groupbuy':($aGoods['act_type']=='score'?'scorebuy':($aGoods['act_type']=='auction'?'auction':$aGoods['act_type']));
    ...
    public function activity_detail($params){
        ...
        $aGoods['act_type'] = $app_id=='groupbuy'?'group':($app_id=='scorebuy'?'score':($app_id=='auction'?'auction':$app_id));
...
group by  comment_id,i.goods_id order by time desc limit {$offset},{$limit}";
    ...
    function get_normal(){
        ...
        $sql = "select g.goods_id,g.name,g.price,g.mktprice,g.store_id,g.buy_m_count,g.freight_bear,g.act_type,s.area,0 as promotion_id,s.store_name,g.udfimg,g.thumbnail_pic,g.image_default_id
        from sdb_b2c_goods as g
        left join sdb_business_storemanger as s on g.store_id=s.store_id
        where g.act_type='normal'
        order by g.view_count desc ";
        $aOrderBy= $obj_goods->orderBy();//
        $orderBy=empty($params['orderby'])?1:intval($params['orderby']);
        ...
        foreach($data as $key => &$product){
            ...
            if($params['images']){
                $image_data = $objImage->getList("attach_id,image_id",array("target_id"=>$product['goods_id'],'target_type'=>'goods'),0,$params['images'],"attach_id asc");
                $product['images'] = null;
                foreach($image_data as $row){
                    $product['images'][] = $this->get_img_url($row['image_id'],$picSize);
                }
            }
    ...
    function get_hot(){
        ...
        $picSize = in_array(strtolower($params['picSize']), array('cl', 'cs'))?strtolower($params['picSize']):'cl';
        $obj_goods = app::get('b2c')->model('goods');
        $sql = "select g.goods_id,g.name,IFNULL(pp.p_price,g.price) as price,g.mktprice,g.store_id,g.buy_m_count,g.freight_bear,g.act_type,s.area,pp.ref_id as promotion_id,s.store_name,g.udfimg,g.thumbnail_pic,g.image_default_id
        from sdb_b2c_goods as g
        left join sdb_business_goods_promotion_price as pp on g.goods_id=pp.goods_id and g.act_type=pp.p_type
        left join sdb_business_storemanger as s on g.store_id=s.store_id
        order by g.view_count desc limit {$offset},{$limit}
        ";
        /*$aOrderBy= $obj_goods->orderBy();//
        $orderBy=empty($params['orderby'])?1:intval($params['orderby']);
        if(!isset($aOrderBy[$orderBy])){
        ...
        $data = $obj_goods->db->select($sql);
        $obj_app = app::get('auction');
        if($obj_app->is_installed()){
            $obj_apply = $obj_app->model('groupapply');
            $obj_log = $obj_app->model('activitylog');
        }
        ...
        foreach($data as $key => &$product){
            ...
            unset($product['image_default_id']);
            unset($product['thumbnail_pic']);
            $product['bid_count'] = $product['act_type']=='auction'?$obj_log->count(array('filter_sql'=>"act_id=(select id from {$obj_apply->table_name(1)} where gid={$product['goods_id']} order by id desc limit 0,1)")):0;
        }
        $data = empty($data)?null:$data;
        $this->send(true, $data, app::get('b2c')->_('success'));
    }
