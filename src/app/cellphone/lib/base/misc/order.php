...
class cellphone_base_misc_order extends cellphone_cellphone
{
    ...
    public function create(){
        ...
        /*
        $params['delivery'] = json_decode('{"addr_id":"79","ship_area":"airport:江苏/无锡市/北塘区:1720","ship_addr_area":"XXXXXX","ship_addr":"123","ship_zip":"111111","ship_name":"cam","ship_mobile":"13833333333"}',1);
        $params['payment'] = json_decode('{"currency":"CNY","pay_app_id":"ysepay","dis_point":"100","is_tax":"true","tax_company":"BBB"}',1);
        $params['shipping'] = json_decode('{"38":{"800":{"shipping_id":"1","memo":""}},"3":{"800":{"shipping_id":"10"},"799":{"shipping_id":"8"}}}',1);
        */
        ...
        if(isset($params['status']) && $params['status'] == 'fastbuy'){
            if(isset($params['type']) && $params['type'] == 'spike'){
            ...
            }elseif(isset($params['type']) && $params['type'] == 'auction'){
                ...
                $obj_order_temp = kernel::single("auction_order_create");
            }else{
        ...
        $temp_split_order=kernel::single('b2c_cart_object_split')->split_order($controller,$addr_id,$allCart);
    ...
    function get_order_data($aCart,$postData,$store_id,&$msg,$arrMember,$obj_order_create){
        ...
        //提交订单 app接口 增加 姓、名、护照、航班号、送达日等相关字段
        if(isset($postData['region_type'])){
            $order_data['region_type']=$postData['region_type'];
        }
        if(isset($postData['firstname'])){
            $order_data['firstname']=$postData['firstname'];
        }
        if(isset($postData['lastname'])){
            $order_data['lastname']=$postData['lastname'];
        }
        if(isset($postData['passport'])){
            $order_data['passport']=$postData['passport'];
        }
        if(isset($postData['flight_no'])){
            $order_data['flight_no']=$postData['flight_no'];
        }
        if(isset($postData['hope_service_date'])){
            $order_data['hope_service_date']=$postData['hope_service_date'];
        }
        if(isset($postData['departure_time'])){
            $order_data['departure_time']=$postData['departure_time'];
        }
    ...
    function get_post_cart($aCart,$sOrder,$postData,$oShipping){
        ...
        //提交订单 app接口 增加 姓、名、护照、航班号、送达日等相关字段
        if(isset($postData['region_type'])){
            $post['region_type']=$postData['region_type'];
        }
        if(isset($postData['firstname'])){
        ...
        if(isset($postData['passport'])){
            $post['passport']=$postData['passport'];
        }
        if(isset($postData['flight_no'])){
            $post['flight_no']=$postData['flight_no'];
        }
        if(isset($postData['hope_service_date'])){
            $post['hope_service_date']=$postData['hope_service_date'];
        }
        if(isset($postData['departure_time'])){
            $post['departure_time']=$postData['departure_time'];
        }
    ...
    function getdetail(){
        ...
        $data = $order_model -> getRow("*", array("order_id"=> $_GET["order_id"]));
        
        $this->send(true,$data,app::get('b2c')->_('订单详情'));
    }
        ...
        if ($b2c_order_cancel->generate($sdf, $b2c_order_cancel, $message))
        {
            ...
            switch($order_info['order_type']){
                ...
                case 'score':
                    ...
                    break;
                case 'auction':
                    $buyMod = app::get('auction')->model('memberbuy');
                    $applyObj = app::get('auction')->model('groupapply');
                    $apply = $applyObj->dump(array('id'=>$order_info['act_id']),'aid,gid,remainnums,nums');
                    if($apply){
                      $buyMod->update(array('effective'=>'false'),array('order_id'=>$order_id));
                ...
                if($refund_data['refund_type'] == '2' && $refund_data['status'] == 'ready'){
                    if($refund_data['pay_app_id'] == 'ysepay'){
                        ...
                        if($result['0'] == "true"){
                            ...
                            if ($ref_rs)
                            {
                                $this->send(true,null,app::get('b2c')->_('确认收货成功！'));
                            ...
                            $this->send(true,null,app::get('b2c')->_('确认收货成功！'.$result['1']));
                        }
                    }else{
                        $obj_refunds = kernel::single("ectools_refund");
                        $ref_rs = $obj_refunds->generate_after(array('refund_id'=>$refund_id,'refund_type'=>'2'));
                        ...
                        if ($ref_rs)
                        {
                            ...
                            $this->send(false,null,app::get('b2c')->_('确认收货成功！更新结算单状态失败！'));
                        }
                    }
                }else{
                    $this->send(false,null,app::get('b2c')->_('结算单类型错误！'));
                }
            }else{
                $this->send(false,null,app::get('b2c')->_('该订单已经结算过，结算单生成失败！'));
            }

        }
        else
        {
            $this->send(false,null,app::get('b2c')->_('确认收货失败！'));
        }
    }
}