...
class cellphone_finder_column{
    ...
    function column_edit($row){
	    ...
	    $html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_column','act'=>'edit','column_id'=>$row['column_id']) ) .'" >'.app::get('b2c')->_('编辑').'</a>&nbsp;&nbsp;';
			
		if($row['is_active']=='false'){
				$html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_column','act'=>'openActivity','column_id'=>$row['column_id']) ) .'" >'.app::get('b2c')->_('开启').'</a>&nbsp;&nbsp;';
		}else{
				$html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_column','act'=>'closeActivity','column_id'=>$row['column_id']) ) .'" >'.app::get('b2c')->_('关闭').'</a>&nbsp;&nbsp;';
			}
	  }
        return $html;
