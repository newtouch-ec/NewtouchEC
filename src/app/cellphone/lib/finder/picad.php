<?php
 class cellphone_finder_picad{
    ...
    function column_edit($row){
	    ...
	    $html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_picad','act'=>'edit','id'=>$row['id']) ) .'" >'.app::get('b2c')->_('编辑').'</a>&nbsp;&nbsp;';
			
		if($row['is_active']=='false'){
				$html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_picad','act'=>'openActivity','id'=>$row['id']) ) .'" >'.app::get('b2c')->_('开启').'</a>&nbsp;&nbsp;';
		}else{
				$html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_picad','act'=>'closeActivity','id'=>$row['id']) ) .'" >'.app::get('b2c')->_('关闭').'</a>&nbsp;&nbsp;';
			}
	  }
        return $html;
