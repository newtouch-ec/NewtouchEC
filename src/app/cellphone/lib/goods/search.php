...
class cellphone_goods_search extends cellphone_cellphone
{
    ...
    private function search($params){
        ...
        // modified by cam begin 2014.03.20
        if(isset($this->params['price_from'])){
            $filter['price'][0] = floatval($this->params['price_from']);
        }
        if(isset($this->params['price_to'])){
            $filter['price'][1] = floatval($this->params['price_to']);
        }
        if(!empty($this->params['loc'])){
            $filter['loc']=$this->params['loc'];
        }
        if(!empty($this->params['real_cat'])){
            $filter['cat_id']=$this->params['real_cat'];
        }
        // modified by cam end
        ...
        if($res === false){            
            ...
            if($params['brand_id']){
                $tmp_filter['str_where']=$tmp_filter['str_where'].' and brand_id='. $params['brand_id'];
            }
        ...
        foreach($result['data'] as $key => &$value){
            $value['bid_count'] = $value['act_type']=='auction'?$obj_log->count(array('filter_sql'=>"act_id=(select id from {$obj_apply->table_name(1)} where gid={$value['goods_id']} order by id desc limit 0,1)")):0;
            $aCat[$value['cat_id']] = null;
            unset($value['cat_id']);
        }
        $result['category'] = app::get('b2c')->model('goods_cat')->getList('cat_id,cat_name',array('cat_id'=>array_keys($aCat)),0,-1);
        // modified by cam end
    ...
    protected function get_filter(){
        if($this->params['cat_id']){
            ...
            if(!cachemgr::get('cellphone_goods_virtual_cat_'.intval($vcatid), $vcat)){
                cachemgr::co_start();
                $vcat = $virCatObj->getList('cat_id,cat_path,filter,cat_name',array('cat_id'=>intval($vcatid)));
