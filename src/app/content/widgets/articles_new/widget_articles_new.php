<?php
    ...
    function article_new_show(&$smarty, $tmp, $setting, &$html, $lv=0, &$limit) {
        ...
        if($setting['shownode'] && $lv!=0) {
            ...
            $html[] = article_new_html($lv, $url, $tmp['info']['node_name'],$typeClass,$tmp['info']['node_id'],$smarty);
        }
        ...
        if($tmp['article']) {
            ...
            foreach ($tmp['article'] as $idx=>$row) {
                ...
                if($limit>0){
                    if($idx == 0){
                        $html[$key] = "<li class='article-index-list'><ul>".article_new_html($tmp_lv, $url, $article_title,'article-index',$row['article_id'],$smarty);
                    }elseif($idx == ($len-1)){
                        $html[$key] = article_new_html($tmp_lv, $url, $article_title,'article-index',$row['article_id'],$smarty)."</ul></li>";
                    }else{
                        $html[$key] = article_new_html($tmp_lv, $url, $article_title,'article-index',$row['article_id'],$smarty);
                    }
                }
    ...
    function article_new_html($lv, $url, $name,$type,$article_id=false,$smarty) {
    	$params = $smarty->_request->get_params();

        if($article_id == $params[0]){

            switch ($lv){
    		case 3:
    		return <<<EOF
<li class="subNav{$lv}">
    <a style="color:red" "href="{$url}">{$name}</a>
</li>
EOF;
break;
    		default:
