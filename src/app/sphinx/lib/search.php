...
class sphinx_search implements search_interface_model{
    ...
    function __construct(){
        ...
        $this->max_limit = $searchConf['sphinx_max_limit'] ? $searchConf['sphinx_max_limit'] : 5000;
    ...
    function SetFilter($filter){
        ...
        elseif(empty($filter['price'][0]) && !empty($filter['price'][1])){
            $this->obj->setFilterFloatRange('price',0,floatval($filter['price'][1]),'');
        }elseif(!empty($filter['price'][0]) && empty($filter['price'][1])){
            $this->obj->setFilterFloatRange('price',floatval($filter['price'][0]),10000000000,'');
        }
        unset($filter['price']);
        // modified by cam end
...
//        }
        if(!empty($filter['price'][0]))$data['price'][0] = $filter['price'][0];
        if(!empty($filter['price'][1]))$data['price'][1] = $filter['price'][1];
        // modified by cam end
        return $data;
    }
...
static function _print($dd){
    ...
    function phone_query($queryArr=array()){
        $this->link();
        $this->st=$queryArr['st'];
        $this->SetFilter($queryArr);
        $this->setOrder($queryArr);
    ...
    function phone_query_store($queryArr=array()){
        $this->link();
        $this->st=$queryArr['st'];
        $this->SetFilter($queryArr);
        $this->setOrder($queryArr);
