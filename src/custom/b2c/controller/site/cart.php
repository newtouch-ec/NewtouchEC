...
class b2c_ctl_site_cart extends b2c_frontpage{
    ...
    public function checkout_result($isfastbuy=0)
    {
		...
		foreach($addrlist as $k=>$v){
			...
			//地域路径
			$addrlist[$k]['area_str'] = $area[1];
            //机场 & 酒店 的场合
            if($addrlist[$k]["region_type"] == 2 || $addrlist[$k]["region_type"] == 3){
                $addrlist[$k]["name"] = "";
                //机场的场合
                if($addrlist[$k]["region_type"] == 3){
                    $addrlist[$k]["addr"] = "";
                }
            }
            else{
                $addrlist[$k]["name"] = "(TO:&nbsp;" . $addrlist[$k]["name"] . ")";
            }
    ...
    public function checkout_goodsbody(){
        ...
        if ($this->pagedata['is_empty'])
        {
            ...
            echo "<script>Message.error('<{t}>购物车商品发生变化，重新结算。<{/t}>');window.location.href='".$url."';</script>";
            exit;
        }
    ...
    public function loginBuy($isfastbuy=0)
    {
        ...
        if( $this->check_login() ) {
            ...
            $this->end( true,app::get('b2c')->_('您已经是登录状态！'));
        }
    ...
    private function _cart_main($msg='',$json_type='all') {
       ...
       $this->pagedata['aCart']['promotion_subtotal'] = $this->objMath->number_minus(array($this->pagedata['aCart']['subtotal'], $this->pagedata['aCart']['subtotal_discount']));
            ...
            //add by Seven for BBC150630001-货币千分位符号 start
            $system_money_separator = app::get('b2c')->getConf('system.money.separator');
            //add by Seven for BBC150630001-货币千分位符号 end
            switch ($json_type)
            {
                case 'mini':
                    $arr_json_data = array(
                        'sub_total'=>array(
                            'promotion_subtotal'=>$obj_currency->changer_odr($aCart['promotion_subtotal'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator),
                            //add by Seven for BBC150520001-对应其他币种汇率表示参考价格 start
                            //购物车修改数量时 获取对应的人民币汇率
                            'promotion_subtotal_cny'=>$obj_currency->changer_odr($aCart['promotion_subtotal'],'CNY',false,false,2,$system_money_operation_carryset,$system_money_separator),
                ...
                case 'middle':
                    $arr_json_data = array(
                        'sub_total'=>array(
                            'subtotal_prefilter_after'=>$obj_currency->changer_odr($aCart['subtotal_prefilter_after'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator),
                            'promotion_subtotal'=>$obj_currency->changer_odr($aCart['promotion_subtotal'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator),
                            //add by Seven for BBC150520001-对应其他币种汇率表示参考价格 start
                            //购物车修改数量时 获取对应的人民币汇率
                            'promotion_subtotal_cny'=>$obj_currency->changer_odr($aCart['promotion_subtotal'],'CNY',false,false,2,$system_money_operation_carryset,$system_money_separator),
                    ...
                    if ($this->pagedata['aCart']['discount_amount_order'] > 0)
                        $arr_json_data['sub_total']['discount_amount_order'] = $obj_currency->changer_odr($this->pagedata['aCart']['discount_amount_order'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator);
                    else
                ...
                default:
                    $arr_json_data = array(
                        'success'=> $msg,
                        'sub_total'=>array(
                            'subtotal_prefilter_after'=>$obj_currency->changer_odr($this->pagedata['aCart']['subtotal_prefilter_after'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator),
                            'promotion_subtotal'=>$obj_currency->changer_odr($this->pagedata['aCart']['promotion_subtotal'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator),
                            //add by Seven for BBC150520001-对应其他币种汇率表示参考价格 start
                            //购物车修改数量时 获取对应的人民币汇率
                            'promotion_subtotal_cny'=>$obj_currency->changer_odr($this->pagedata['aCart']['promotion_subtotal'],'CNY',false,false,2,$system_money_operation_carryset,$system_money_separator),
                            //add by Seven for BBC150520001-对应其他币种汇率表示参考价格 end
                        ),
				...
				if ($arrMember['member_id']){
					...
					if( is_array($aData[$key]) ) {
						foreach( $aData[$key] as $_key => $_val ) {
							$coupons = $obj_coupons->getRow('*',array('cpns_id'=>$_val['cpns_id']));
							$aData[$key][$_key]['cpns_name'] = $coupons['cpns_name'];
    ...
    */
	...
	public function save_new_addr(){
        ...
        if(empty($post['addrZip']) && empty($post['addrMobile'])){
            echo json_encode(array('status'=>'many','msg'=>app::get('b2c')->_('手机号码和电话号码不能都为空')));exit;
        }
		$obj_member_addrs = $this->app->model('member_addrs');
		$arrMember = $this->get_current_member();
		if(!$arrMember['member_id']){
			$data['status'] = 'nologin';
			$data['msg'] = app::get('b2c')->_('没有登录');
		}else{
			$nums = $obj_member_addrs->count(array('member_id'=>$arrMember['member_id']));
			if($nums>=6){
				echo json_encode(array('status'=>'many','msg'=>app::get('b2c')->_('您最多可添加6个收货地址')));exit;
			}
			$member_id = $arrMember['member_id'];
			...
			if($check[0]){
				echo json_encode(array('status'=>'like','msg'=>app::get('b2c')->_('相同的收货地址')));exit;
			}
			$item['zip'] = $post['addrZip'];
		    $item['tel'] = $post['addrTel'];
			$item['mobile'] = $post['addrMobile'];
			$item['region_type'] = $post['region_type'];
			$ret = $obj_member_addrs->insert($item);
			if($ret){
				$data['status'] = 'succ';
				$data['msg'] = app::get('b2c')->_('更新成功');
				$data['addr'] = $ret;
			}else{
				$data['status'] = 'failed';
				$data['msg'] = app::get('b2c')->_('更新失败');
			}
		}
	...
	public function check_orders(){
		...
		if($aCart['object']['goods']){
			foreach($aCart['object']['goods'] as $key=>$val){
				//数量组件
				if(array_key_exists($val['params']['goods_id'],$goods_list)){
					$goods_list[$val['params']['goods_id']] += (int)$val['quantity'];
				}else{
					$goods_list[$val['params']['goods_id']] = (int)$val['quantity'];
				}
				//订单金额组件
				/*if(array_key_exists($val['store_id'],$order_list)){
					$order_list[$val['store_id']] += (int)$val['quantity']*(float)$val['obj_items']['products'][0]['price']['buy_price'];
				}else{
					$order_list[$val['store_id']] = (int)$val['quantity']*(float)$val['obj_items']['products'][0]['price']['buy_price'];
				}*/
			}
		...
		if($goods_list){
			foreach($goods_list as $g_id=>$quantity){
				$maxbuy = $objGoods->dump(array('goods_id'=>$g_id),'max_buy,name');
				if((int)$maxbuy['max_buy'] > 0){
					if((int)$maxbuy['max_buy'] < (int)$quantity){
						$error_json = array(
							'status'=>false,
							'error'=>app::get('b2c')->_("对不起，您购买的商品".$maxbuy['name']."超过限购个数".$maxbuy['max_buy']."，请修改数量！"),
						);
		...
		/*$max_buy_num_per_order_switch = app::get('b2c')->getConf('goods.max_buy_num_per_order_switch');
		if($max_buy_num_per_order_switch == 'true'){
			if($order_list){
				$max_money_per_order = app::get('b2c')->getConf('shops.max_money_per_order');
				foreach($order_list as $store_id=>$tatal_money){
					if((float)$max_money_per_order < (float)$tatal_money){
						$error_json = array(
							'status'=>false,
							'error'=>app::get('b2c')->_("对不起，您的订单总金额超出限制".$max_money_per_order."，请控制单张订单的金额！"),
						);
						echo json_encode($error_json);exit;
					}
				}
