...
class b2c_ctl_site_member extends b2c_frontpage{
    ...
    function __construct(&$app,$verify=true){
        ...
        if($minfo['member_id']){
            ...
            //卖家限制
            if($is_seller && (get_class($this)=='b2c_ctl_site_member' || get_class($this)=='business_ctl_site_favorite') && !in_array($this->action,array('checkOldPwd'))){
    ...
    private function get_cpmenu(){
        ...
        $arr_bases = array(
            array('label'=>app::get('b2c')->_('我的收藏夹'),
                'mid'=>0,
                'items'=>array(
                    array('label'=>app::get('b2c')->_('宝贝收藏'),'app'=>'b2c','ctl'=>'site_member','link'=>'favorite'),
            ...
            array('label'=>app::get('b2c')->_('账户中心'),
                ...
                'items'=>array(
                    ...
                    array('label'=>app::get('b2c')->_('账户安全'),'app'=>'b2c','ctl'=>'site_member','link'=>'security'),
       ...
       #获取会员基本信息
		...
		$member_sdf = array(
			...
			'contact'=>array(
                ...
                'passport' => $member_info[0]['passport'],//add
			...
			'login_time'=>$member_info[0]['login_time'],
    ...
    function setting(){
        ...
        foreach((array)$attr as $key=>$item){
            ...
            if($item['attr_group'] == 'defalut'){
             switch($attr[$key]['attr_column']){
                    ...
                    //add by ql
                    case 'firstname':
                    $attr[$key]['attr_value'] = $mem['firstname'];
					break;
					case 'lastname':
                    $attr[$key]['attr_value'] = $mem['lastname'];
					break;
					case 'passport':
                    $attr[$key]['attr_value'] = $mem['passport'];
					break;
                    //end
       ...
       // echo '<pre>';print_r($attr);exit;
        ...
        $this->pagedata['firstname'] = $mem['contact']['firstname'];
        $this->pagedata['lastname'] = $mem['contact']['lastname'];
        $this->pagedata['passport'] = $mem['contact']['passport'];
        $this->output();
    }
    ...
    function save_setting(){
        ...
        if($_POST['contact']['phone']['mobile'] && !preg_match('/^1[34578][0-9]{9}$/',$_POST['contact']['phone']['mobile'])){
	...
	public function orders_member()
	{
       ...
       if($type=='notify')
        ...
        foreach($aData['data'] as $k=>$v) {
            ...
            $seller_id = $obj_strman->getRow('account_id,store_idcardname,store_name,status',array('store_id'=>$v['store_id']));
            ...
            $aData['data'][$k]['seller_store_name'] = $seller_id['store_name'];
			$aData['data'][$k]['seller_store_status'] = $seller_id['status'];
...
//print_r($aData['data']);
    ...
    public function orderdetail($order_id=0)
    {
		...
		$seller_id = $obj_strman->getRow('account_id,store_idcardname,store_name,status',array('store_id'=>$sdf_order['store_id']));
		$seller_name = $obj_members->getRow('login_name',array('account_id'=>$seller_id['account_id']));
		$this->pagedata['order']['seller_name'] = $seller_name['login_name'];
		$this->pagedata['order']['seller_real_name'] = $seller_id['store_idcardname'];
		$this->pagedata['order']['seller_store_name'] = $seller_id['store_name'];
		$this->pagedata['order']['seller_store_status'] = $seller_id['status'];
    ...
    public function balance($nPage=1)
    {
        ...
        $params['data'] = $mem_adv->getList('*',array('member_id' => $this->member['member_id']),$aPage['start'],$this->pagesize, 'log_id desc');
    ...
    function security($type = ''){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
         $this->path[] = array('title'=>app::get('b2c')->_('修改密码'),'link'=>'#');
          $GLOBALS['runtime']['path'] = $this->path;
        $obj_member = &$this->app->model('members');
        $this->pagedata['mem'] = $obj_member->dump($this->app->member_id,'*',array(':account@pam'=>array('*')));
        
        $path='site/security/base.html';
        switch ($type) {
            case 'email':
                $path='site/security/email.html';
                break;
            case 'question':
                $path='site/security/question.html';
                break;
            case 'phone':
                $path='site/security/phone.html';
                break;
            case 'password':
                $path='site/security/password.html';
                break;
    ...
    public function save_security_question(){
        $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'security'));
        $obj_member = &$this->app->model('members');
        $result=$obj_member->check_oldpasswd($this->app->member_id,$_POST['old_passwd'],$msg);
        if(!$result){
    ...
    function do_verify_email(){
        ...
        $return_token = $data[0];
        $this->begin('');
        $veremail_model = &$this->app->model('member_veremail');
        $member_model = &$this->app->model('members');

        if($return_token){
            if($_SESSION['account'][pam_account::get_account_type($this->app->app_id)]){
                $memberid = $this->app->member_id;
                $veremail_data['status'] = 'y';
                $veremail_info = $veremail_model->getList('*',array('member_id'=>$memberid,'status'=>'n','token'=>$return_token));
                $veremail_model->update($veremail_data,array('member_id'=>$memberid,'status'=>'n','token'=>$return_token));

                $data['email'] = $veremail_info[0]['email'];
                $data['verify_email']='Y';
                $res = $member_model->update($data,array('member_id'=>$memberid));
                if($res){
                    $this->splash('success', $url , app::get('b2c')->_('邮箱验证成功！'),'','');
                }else{
                    $this->splash('failed', $url,app::get('b2c')->_('邮箱验证失败！'),'','');
                }
            }else{
                $this->splash('failed', $url,app::get('b2c')->_('验证邮件已失效，请重新验证！'),'','');
            }
        }else{
            $obj_member = &$this->app->model('members');
            $mem=$obj_member->dump($this->app->member_id);
            $result=$obj_member->check_oldpasswd($this->app->member_id,$_POST['old_passwd'],$msg);
            if(!$result){
                $this->splash('failed','',$msg,'','',true);
            }
            if($_POST['old_email']==$_POST['email'] && $mem['verify_email']=='Y'){
                $this->splash('failed','','该邮箱已经验证，请勿再次验证。','','',true);
            }
            if($obj_member->is_exists_email($_POST['email'],$this->app->member_id)){
                $this->splash('failed','','该邮箱已经被使用，请更换邮箱。','','',true);
            }
            $_POST = $this->check_input($_POST);
            ...
            //生成token
            $str = $this->randString();
            $token = md5($_POST['member_id'].$str);
            $_POST['token'] = $token;

            $veremail_model->save($_POST);

            $rows = app::get('pam')->model('account')->getList('*', array('account_type'=>'member', 'account_id'=>$_POST['member_id']));

            $login_name = $rows[0]['login_name'];
            $turl = $this->gen_url(array('app'=>'b2c','ctl'=>"site_member",'act'=>"do_verify_email",'full'=>1,'arg'=>$token));
           
            if($this->send_email($login_name, $_POST['email'], $turl,$this->app->member_id)){
                $this->splash('success', $url , app::get('b2c')->_('邮件已经成功发送到您的邮箱，请及时查收'),'','',true);
            }else{
                $this->splash('failed','',app::get('b2c')->_('邮件发送失败，请稍后再试！'),'','',true);
            }
        }

    }

    function send_email($login_name, $user_email, $url,$member_id){
        $ret = $this->app->getConf('messenger.actions.account-verifyEmail');
        $ret = explode(',', $ret);
        ...
        $data['sendemailcodetype'] = 'userRegister';
        $data['shopname']=$this->app->getConf('system.shopname');
        $obj_account = &$this->app->model('member_account');
        return $obj_account->fireEvent('verifyEmail', $data, $member_id);
    }
    ...
    function do_verify_mobile(){
        $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'security'));

        $this->begin(array('app'=>'b2c','ctl'=>'site_member','act'=>'security'));
        if($_POST['contact']['phone']['mobile'] && !preg_match('/^1[34578][0-9]{9}$/',$_POST['contact']['phone']['mobile'])){
            $this->splash('failed', '', app::get('b2c')->_('手机输入格式不正确'),'','',true);
        }
        $member_model = &$this->app->model('members');
        $result=$member_model->check_oldpasswd($this->app->member_id,$_POST['old_passwd'],$msg);

        if(!$result){
            $this->splash('failed','',$msg,'','',true);
        }
        $mem=$member_model->dump($this->app->member_id);
        if($_POST['old_mobile']==$_POST['contact']['phone']['mobile'] && $mem['verify_mobile']=='Y'){
            $this->splash('failed','','该手机已经验证，请勿再次验证。','','',true);
        }
        //验证手机短信验证码
       if(!$member_model->check_mobilecode($_POST['mobileverifycode'], $msg)){
            $this->splash('failed', '','手机验证码错误！','','',true);
        }
        ...
        $_POST['member_id'] = $this->app->member_id;
        $_POST['verify_mobile'] ='Y';
        if($member_model->save($_POST)){
            $this->splash('success', $url , app::get('b2c')->_('手机验证成功'),'','',true);
        }else{
            $this->splash('failed', $url , app::get('b2c')->_('手机验证失败'),'','',true);
        }
    ...
    function getMobileCode(){
        ...
        if($objmember->check_mobile($mobile, $message,$this->app->member_id)){
            //验证是否是恶意请求验证码
            $request_time = time();
    ...
    function receiver(){
        ...
        //add by ql
        $minfo=$this->get_current_member();
        $this->pagedata['member_info'] = $minfo;
        $this->output();
    }
	...
	function get_region_path(){
		$region_path = array();
		$localName = trim($_POST['postCode']);
		$row_regions = app::get('ectools')->model('regions');
		$options = $row_regions->getParentByLocalName($localName);
		$region_path = explode(',',$options['region_path']);
		echo $options['region_path'];exit;
	}
	
	//根据州id,取得城市列表
	function get_p_regionlist(){
		$region_path = array();
		$p_region_id = trim($_POST['p_region_id']);
		$regions = app::get('ectools')->model('regions');
		$options = $regions->getRegionBypRegionId($p_region_id);
	...
	function get_addr(){
		...
		$regions = app::get('ectools')->model('regions');
		$options = $regions->getRegionByRegionId($region_id);
		echo json_encode($options);exit;
	}
    
    //根据电话号码，检索地域详细信息
    function get_region_addr(){
        $parent_list = array();
        
		$phone = trim($_POST['phone']);
		$regions = app::get('ectools')->model('regions');
		$row = $regions->getRow("*", array("p_1"=>$phone));
        
        //检索到具体酒店的场合
        if($row){
            $parent_path = explode(",", $row["region_path"]);
            
            //酒店详细信息
            $parent_list[] = $row;
            
            //循环取得二级/三级地域列表
            for ($i=0; $i <= count($parent_path); $i++)
            {
                if($parent_path[$i] == "") continue;
                
                //上级地域信息
                $parent_list[] = $regions->getList("*", array("p_region_id"=>$parent_path[$i]));
            }
        }
        
        echo json_encode($parent_list);exit;
    }
    ...
    function insert_rec(){
        ...
        $obj_member = &$this->app->model('members');
        if(!$obj_member->isAllowAddr($this->app->member_id)){
             $this->splash('failed','',app::get('b2c')->_('不能新增收货地址'),'','',false);
        }
        $aData = $this->check_input($_POST);
        if($obj_member->insertRec($aData,$this->app->member_id,$message)){
             $this->splash('success',$url,$message,'','',false);
            }
        else{
            $this->splash('failed','',$message,'','',false);
        }
    ...
    function save_rec(){
        ...
        if($obj_member->check_addr($_POST['addr_id'],$this->member['member_id']))
        {
            ...
            if($obj_member->save_rec($aData,$this->app->member_id,$message)){
                $this->splash('success',$back_url,app::get('b2c')->_('修改成功'),'','',false);
    ...
    function coupon($nPage=1) {
        ...
        if ($aData) {
            foreach ($aData as $k => $item) {
				...
				$coupons = $obj_coupons->getRow('*',array('cpns_id'=>$item['cpns_id']));
				$store_id = intval(trim($coupons['store_id'],','));
				
				$store = $obj_storemanger->getRow('*',array('store_id'=>$store_id));
				$aData[$k]['store_name'] = $store['store_name'];
				}
        }
    ...
    public function gofinish()
    {
        ...
        if ($b2c_order_finish->generate($sdf, $this, $message))
        {
            ...
            if($returns[0]['amount']){
                ...
                if($money['score_g'] > 0){
                    $sdf['money'] = $sdf['money']-($money['score_g'])*$point_deductible_valu;
                ...
                if($money['discount_value'] > 0 && $money['score_g'] > 0){
                    ...
                    $sdf['money'] = $money['payed']-$profit-($money['score_g'])*$point_deductible_valu;
                    $sdf['score_cost'] = ($money['score_g'])*$point_deductible_valu;
                }else{
                    $sdf['money'] = $money['payed']-$profit; 
                }
