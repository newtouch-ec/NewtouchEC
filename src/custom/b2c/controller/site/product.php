...
class b2c_ctl_site_product extends b2c_frontpage{
    ...
    public function index() {
        ...
        if(!$this->id){
            ...
            $rs = $objGoods->dump(array('goods_id'=>$this->id),'goods_id,store_id');
            if(!$rs || empty($rs)){
                $this->splash('failed', 'back', app::get('b2c')->_('无效商品！<br>可能是商品未上架'));
            }

            //判断店铺是否有效
            $mdl_storemanager = app::get('business')->model('storemanger');
            $store_rs = $mdl_storemanager->checkStoreActive($rs['store_id']);
            if($store_rs == false){
                $this->splash('failed', 'back', app::get('b2c')->_('无效店铺的商品！'));
            }
        ...
        //添加max_buy字段
        $aGoods_list = $objGoods->getList("store_id,goods_state,buy_m_count,fav_count,freight_bear,comments_count,avg_point,goods_id,name,bn,price,cost,mktprice,marketable,store,store_freeze,notify_num,score,weight,unit,brief,image_default_id,udfimg,thumbnail_pic,small_pic,big_pic,min_buy,max_buy,package_scale,package_unit,package_use,score_setting,nostore_sell,goods_setting,disabled,spec_desc,adjunct,p_1,p_2,p_3,p_4,p_5,p_6,p_7,p_8,p_9,p_10,p_11,p_12,p_13,p_14,p_15,p_16,p_17,p_18,p_19,p_20,p_21,p_22,p_23,p_24,p_25,p_26,p_27,p_28,p_29,p_30,p_31,p_32,p_33,p_34,p_35,p_36,p_37,p_38,p_39,p_40,p_41,p_42,p_43,p_44,p_45,p_46,p_47,p_48,p_49,p_50,brand_id,type_id,cat_id,seo_info,act_type",array('goods_id'=>$gid,'store_id|than'=>0));
        ...
        if(!is_array($aGoods['adjunct'])){
            ...
            //配件数量 逻辑调整
            $rs = kernel::single('b2c_goods_detail_adjunct')->_get_goods($gid);
            $adjunct_goods_num = 0;
            foreach($rs['adjunct'] as $goods_adjunct_key => $goods_adjunct_value){
                if($goods_adjunct_value['items']){
                    $adjunct_goods_num += count($goods_adjunct_value['items']);
                }
            }
       ...
       $this->pagedata['specShowItems'] =$specValue;
        ...
        $ret =array(
            ...
            'separator'=>$this->app->getConf('system.money.separator'),
            'sign' => $cur_info['sign']
        );
    ...
    */
    ...
    function get_dlytype(){
        ...
        $objGoods = &$this->app->model('goods');
        $aGoods = $objGoods->getList("store_id,goods_state,weight,goods_id",array('goods_id'=>$gid));
