...
class b2c_cart_object_goods implements b2c_interface_cart_object{
    ...
    public function check_store($arr_data, $arr_carts, &$msg='',$_check_adjunct=true)
    {
        ...
        //商品页添加到购物车时超过购买上限的情况报错误消息框
        if ( $this->app->getConf('goods.max_buy_num_per_order_switch') ) {
            $max_buy = $this->o_goods->getList('max_buy',array('goods_id'=>$arr_data['goods']['goods_id']));
            if ( $max_buy[0]['max_buy'] > 0 ){
                if ( $max_buy[0]['max_buy'] <= $arr_carts[$product_id]['quantity'] ){
                    $msg =app::get('b2c')->_('购物车内该商品数量已达限购数！');
                    echo json_encode( array('error'=>$msg) );exit;
                }
                else if ( $max_buy[0]['max_buy'] - $arr_carts[$product_id]['quantity'] < $aData['quantity'] ){
                    $aData['quantity'] = $max_buy[0]['max_buy'] - $arr_carts[$product_id]['quantity'];
                }
            }
        }
    ...
    private function _check($aData, $_check_adjunct=true) {
        ...
        if( !isset($this->check_goods_info[$goods_id]) )
        //mod by Seven for BBC150518001-商品购买上限 start
        //$this->check_goods_info[$goods_id] = $oSG->getList('goods_id, store,nostore_sell, marketable', array('goods_id'=> "$goods_id"));
        $this->check_goods_info[$goods_id] = $oSG->getList('goods_id, store,nostore_sell, marketable,max_buy', array('goods_id'=> "$goods_id"));
        //mod by Seven for BBC150518001-商品购买上限 end
        ...
        if ( !$aGoods['nostore_sell'] ) {
            if(empty($arr_product['store'])){
                if(isset($arr_product['store']) && $arr_product['store']!=='' ) {
                    return $this->get_error_msg(app::get('b2c')->_('该商品已无库存！')); //库存0
                }
            // 检测是否够库存
            } else if($aData['quantity']>$arr_product['store']) {
                return $this->get_error_msg(app::get('b2c')->_('购买数量超出库存'));
            }
        }
        ...
        //购物车页修改商品购买数量时超过限购数
        if ( $this->app->getConf('goods.max_buy_num_per_order_switch') ) {
            if( $aGoods['max_buy'] != '' && $aGoods['max_buy'] != 0 && $aData['quantity'] > $aGoods['max_buy']) {
                return $this->get_error_msg(app::get('b2c')->_('购买数量超出购买上限'));
            }
        }
        //mod by Seven for BBC150518001-商品购买上限 end
    ...
    protected function _check_goods( &$aData, $arr_goods_id ) {
        ...
        $arr = $this->o_goods->getList('goods_id, store,nostore_sell, marketable,max_buy', array('goods_id'=> $arr_goods_id));
        //mod by Seven for BBC150518001-商品购买上限 end
        ...
        foreach($arr as $row) {
            ...
            //加入的商品数量超过上限时设置为上限数
            if ( $this->app->getConf('goods.max_buy_num_per_order_switch') ) {
                if( $row['max_buy']!='' && $row['max_buy']>0 && $aData[$key]['quantity']>$row['max_buy']) {
                    $aData[$key]['quantity'] = $row['max_buy'];
                }
            }
            //add by Seven for BBC150518001-商品购买上限 end
    ...
    protected function _check_products( &$aData, $arr_products_id, $cur_type='' ) {
        ...
        foreach($arr as $row) {
            ...
            if( !$this->nostore_sell[$row['goods_id']] ) {
                ...
                //加入的商品数量超过上限时设置为上限数
                if ( $this->app->getConf('goods.max_buy_num_per_order_switch') ) {
                    $rows = $this->o_goods->getList('max_buy', array('goods_id'=> $aData[0]['params']['goods_id']));
                    if( $rows[0]['max_buy']!='' && $rows[0]['max_buy']>0 && $aData[$key]['quantity']>$rows[0]['max_buy']) {
                        $aData[$key]['quantity'] = $rows[0]['max_buy'];
                        $arr_save = array('obj_ident'=>$aData[$key]['obj_ident'],'quantity'=>$rows[0]['max_buy']);
                        $this->app->model('cart_objects')->save( $arr_save );
                    }
                }
                //add by Seven for BBC150518001-商品购买上限 end
    ...
    private function _get($aData) {
        ...
        foreach($aData as $key => &$row) {
            ...
            if(isset($tmp_products_store[$product_id])) {
				...
				$tmp_products_store[$product_id]['real_store'] = $store_id['store'] - $store_id['store_freeze'];
            } else {
                $tmp_store = array(
    ...
    private function get_product_adjunct($pid=0, $adjunct=array(), $quantity=0, &$tmp_adjunct_name=array(), &$row, &$tmp_products_store) {
        ...
        $tmp_store = array(
						...
						'real_store' => &$tmp_products_store[$pid]['real_store'],
                    );
            ...
            if( $ident['index'] ) {
                ...
                foreach( (array)$row as $val ) {
                    if( $ident['id']==$val['product_id'] ) {
                        return array('buy_price'=>($o_currency->changer_odr($val['subtotal'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator)),'consume_score'=>(float)($val['gain_score']*$val['quantity']));
                    }
                }
            } else {
                return array('buy_price'=>$o_currency->changer_odr($row['obj_items']['products'][0]['price']['buy_price']*$row['quantity'],$_COOKIE["S"]["CUR"],false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator), 'consume_score'=>(float)($row['obj_items']['products'][0]['gain_score']*$row['quantity']));
            }
        }
    }
