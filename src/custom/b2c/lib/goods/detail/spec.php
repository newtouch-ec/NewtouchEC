...
//        $render->pagedata['goods'] = $aGoods;
        ...
        $cny_rate= $cur->getcur('CNY');
        $ret['cny_rate'] = $cny_rate['cur_rate'];
        //add by Seven for BBC150520001-对应其他币种汇率表示参考价格 end
        
        unset($cur_info);
        if( $aGoods['spec'] && is_array($aGoods['spec']) )  {
            foreach( $aGoods['spec'] as $sk=>$row ) {
                $option = $row['option'];

                if( $option && is_array($option) ) {
                    foreach( $option as $opk=>$img ) {

                        //展示规格图片 展示首张商品关联图片。
                        if(isset($img['spec_image']) && empty($img['spec_image'])){
                            $tmp_simg = explode(',',$img['spec_goods_images']);
                            $aGoods['spec'][$sk]['option'][$opk]['spec_image'] = $tmp_simg[0];
                        }
        ...
        $render->pagedata['goods']['system_money_separator'] = app::get('b2c')->getConf('system.money.separator');
