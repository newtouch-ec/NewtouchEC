    ...
    public function save(&$sdf, &$msg='')
    {
		...
		if($sdf['order_objects']){
			foreach($sdf['order_objects'] as $key=>$val){
				if(array_key_exists($val['goods_id'],$goods_list)){
					$goods_list[$val['goods_id']] += (int)$val['quantity'];
				}else{
					$goods_list[$val['goods_id']] = (int)$val['quantity'];
				}
			}
			...
			foreach($goods_list as $g_id=>$quantity){
				...
				if((int)$maxbuy['max_buy'] > 0){
					if((int)$maxbuy['max_buy'] < (int)$quantity){
						$msg = app::get('b2c')->_("对不起，您购买的商品".$maxbuy['name']."超过限购个数".$maxbuy['max_buy']."，请返回购物车修改！");
						return false;
					}
				}
			}
		...
		$max_money_per_order = $objStore->dump(array('store_id'=>$sdf['store_id']),'order_max_money');
		$flag_order = false;
		if((float)$max_money_per_order['order_max_money'] > 0){
			if((float)$max_money_per_order['order_max_money'] < ((float)$sdf['cur_amount']-(float)$sdf['shipping']['cost_shipping'])){
				$msg_text = $max_money_per_order['order_max_money'];
				...
				$msg_text = $obj_currency->changer_odr($msg_text,'',false,false,$system_money_decimals,$system_money_operation_carryset,$system_money_separator);
				$msg = app::get('b2c')->_("对不起，您有订单总金额超出限制".$msg_text."，请返回购物车控制单张订单的金额！");
				return false;
			}else{
				$flag_order = true;
			}
