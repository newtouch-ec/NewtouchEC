        ...
        foreach($order_ids as $key=>$val){
            ...
            if($tag){
                $sdf_order_log = array(
                    ...
                    'behavior' => 'delivery',
                    'result' => 'SUCCESS',
                    'log_text' => app::get('b2c')->_('系统已发货，无需物流'),
                    'addon' => $log_addon,
                );
            }else{
                $sdf_order_log = array(
    ...
    private function __order_payment($rel_id, &$sdf, &$status='succ',&$msg='',&$refund_status=false,&$refund_type='0')
    {
        ...
        if ($sdf_order)
        {
            ...
            /******************卖家提醒 begin ***************************/
                ...
                //店铺信息  根据店铺id 查找member_id  向该store_id对应的member_id进行邮件短信提醒。
                $storeInfo = $obj_store->dump($store_id);
                //error_log(var_export($storeInfo,true),3,DATA_DIR.'/storeInfo.txt');
                    if(!empty($storeInfo)){
                        $storeData = array(
                        'order_id' => $rel_id,
                        'paytime' =>  date('Y-m-d', time()),
                        'money' => $sdf['cur_money'],
                        'email' => $storeInfo['finance_email'],
                         'member_id' => $storeInfo['account_id'],
                        'pay_status' => $sdf['pay_status'],
                        'pay_account' => $arr_members['pam_account']['login_name'],
                        'is_frontend' => true,
                    );
                  //       error_log(var_export($storeData,true),3,DATA_DIR.'/storeData.txt');
                }
              $obj_orders->fireEvent('payedbusiness', $storeData, $storeInfo['account_id']);
            /******************卖家提醒 end ***************************/
        }
        else
        {
...
//                $count_stat = unserialize($items['goods']['count_stat']);
                ...
                $goods_row = $objGoods->getList('count_stat',array('goods_id'=>$items['goods']['goods_id']));
                $count_stat = unserialize($goods_row[0]['count_stat']);
