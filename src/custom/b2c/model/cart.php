...
class b2c_mdl_cart extends dbeav_model{
    ...
    public function get_objects($aData=array(),$aResult = array(),$aConfig = array()) {
        foreach(kernel::servicelist('b2c_cart_process_apps') as $object) {
			...
			//add by libaoji 团购秒杀拍卖不能加入购物车
            if($object->get_order()==95){
                continue;
            }
   ...
   public function count_objects(&$aData) {
       ...
       if( $aData['subtotal']<$aData['discount_amount'] )
		...
		if(is_array($aData['object']['goods'])){
   ...
   // 购物车物品项render
   public function get_item_render($service_key='b2c_cart_render_items_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {;
           if(!is_object($object)) continue;
   ...
   //团购购物车物品项render
   public function get_group_item_render($service_key='group_cart_render_items_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {;
           if(!is_object($object)) continue;
           $aResult[$object->index] = (array) $object;
   ...
   //秒杀购物车物品项render
   public function get_spike_item_render($service_key='spike_cart_render_items_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {;
           if(!is_object($object)) continue;
           $aResult[$object->index] = (array) $object;
   ...
   //积分换购购物车物品项render
   public function get_score_item_render($service_key='scorebuy_cart_render_items_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {;
           if(!is_object($object)) continue;
           $aResult[$object->index] = (array) $object;
   ...
   // 购物车物品项render
   public function get_item_goods_render($service_key='b2c_cart_render_items_goods_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {
           if(!is_object($object)) continue;
           $aResult[$object->index] = (array) $object;
   ...
   // 购物车物品项render
   public function get_item_render_view($service_key='b2c_cart_render_items_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {;
           if(!is_object($object)) continue;
   ...
   // 购物车物品项render
   public function get_item_goods_render_view($service_key='b2c_cart_render_items_goods_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {
           if(!is_object($object)) continue;
           if( method_exists($object,'_get_minicart_view') ) {
   ...
   // 优惠项render
   public function get_solution_render($service_key='b2c_cart_render_solutions_apps') {
       $aResult = array();
       foreach(kernel::servicelist($service_key) as $object) {
