...
class b2c_mdl_member_addrs extends dbeav_model{
    ...
    function save(&$data,$mustUpdate=null){
        //日本酒店/旅馆 的场合
        if($data["address_type_edit"] == 2 || $data["address_type_add"] == 2){
            $data["area"]["area_type"] = "hotel";
            $data['region_type'] = 2;
        }
        //日本机场 的场合
        else if($data["address_type_edit"] == 3 || $data["address_type_add"] == 3){
            $data["area"]["area_type"] = "airport";
            $data['region_type'] = 3;
        }
