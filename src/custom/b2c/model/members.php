...
class b2c_mdl_members extends dbeav_model{
     ...
     function getMemberAddr($nMemberId){
            ...
            $str ="
                select 
                	...
                	reg.p_1,
                	reg.p_2,
                	substring_index(addr.area,':', 2)
                from 
                	sdb_b2c_member_addrs addr, 
                	sdb_ectools_regions reg
                where 
                	substring_index(addr.area,':', -1) = reg.region_id
                    and addr.member_id = '$nMemberId'
                ";
    ...
    function save_rec($aData,$nMemberId,&$message){
        ...
        */
        ...
        if($aData['phone']['mobile'] == ''){
            $message = app::get('b2c')->_('手机号码不能为空！');
            return false;
        }
    ...
    public function get_member_info( $member_id ) {
        ...
        $member_sdf = $this->db->selectrow("select m.firstname,m.lastname,m.passport,p.login_name,m.member_id,m.name,m.sex,m.point,m.experience,m.email,m.member_lv_id,cur,advance,m.seller,mobile
        ...
        if( !empty($member_sdf) ) {
            ...
            $this->member_info['firstname'] = $member_sdf['firstname'];
            $this->member_info['lastname'] = $member_sdf['lastname'];
            $this->member_info['passport'] = $member_sdf['passport'];
...
//                    }
        ...
        if($data['reg_type'] == 'mobile'){
            if(trim($data['contact']['phone']['mobile']) == ''){
                ...
                if(!preg_match('/^1[34578][0-9]{9}$/', trim($data['contact']['phone']['mobile']))){
...
//                    }
    ...
    function check_mobile($mobile, &$message, $member_id=null){
        ...
        if(empty($mobile)){
            ...
            return false;
        }elseif(!preg_match("/^1[34578][0-9]{9}$/", $mobile)){
...
//        }
    ...
    */
    ...
    function getPhoto($member_id){
        $member = $this->getList('image',array('member_id'=>$member_id));
        return $member[0]['image'];

    }
    function check_oldpasswd($nMemberId='',$old_passwd='', &$msg){
        if(empty($nMemberId)){
            $msg='用户不存在';
        }
        if(empty($old_passwd)){
        ...
        $use_pass_data['login_name'] = $aMem['pam_account']['login_name'];
        $use_pass_data['createtime'] = $aMem['pam_account']['createtime'];
        $login_password= pam_encrypt::get_encrypted_password(trim($old_passwd), pam_account::get_account_type($this->app->app_id), $use_pass_data);
        if($aMem['pam_account']['login_password']!==$login_password){
            $msg='密码错误，请重新输入密码。';
            return false;
        }
        return true;
