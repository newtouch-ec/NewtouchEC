...
class business_ctl_site_activity extends b2c_frontpage{
    ...
    function attend($type='timedbuy'){
        ...
        if($business_activity&&$type=='timedbuy'){
            $url = $this->router->gen_url(array('app'=>'timedbuy','ctl'=>'site_activity','act'=>'attend'));
        }
		
		//if($type=='package'){
            $url = $this->router->gen_url(array('app'=>'package','ctl'=>'site_activity','act'=>'attend'));
        //}
    ...
    function myAttend(){
		...
		$url = $this->router->gen_url(array('app'=>'package','ctl'=>'site_activity','act'=>'myAttend'));
        kernel::single('base_controller')->splash( 'success',$url );
    }
}