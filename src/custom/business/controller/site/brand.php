...
//       $this->pagedata['issue_type'] = $issue_type;
    ...
    function save(){
      ...
      $brandStatus = $obj_brand->getList('status',array('id'=>$_POST['id'],'store_id'=>$_POST['store_id']));
	  $obj_storegrade = $this -> app_current -> model('storegrade');
	  $stype = $obj_storegrade -> getList('*', array('grade_id' => $shopinfo['store_grade']));

      if ($brandStatus[0]['status'] == '1'){
        $this->splash('failed',$url,app::get('business')->_('此品牌已审核通过，不可编辑！'));
      }
