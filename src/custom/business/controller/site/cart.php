...
class business_ctl_site_cart extends b2c_ctl_site_cart{
    ...
    public function create()
    {
        ...
        //由于订单画面地址切换时，ship_area的值会出现丢失问题，使用ship_area_backup变量值
        $_POST['delivery']['ship_area'] = $_POST["ship_area_backup"];
		...
		if(!$_POST['delivery']['ship_mobile'] && !$_POST['delivery']['ship_tel'] && !$_POST['delivery']['ship_area']){
			$msg .= app::get('b2c')->_("收货信息不能为空！")."<br />";
			eval("\$msg = \"$msg\";");
            $this->end(false, $msg, '',true,true);
		}

        if ((!$_POST['delivery']['ship_email'] && !$arrMember['member_id']) || (!$_POST['delivery']['ship_mobile'] && !$_POST['delivery']['ship_tel']) || $_POST['delivery']['shipping_id'] || !$_POST['payment']['pay_app_id'])
        {	
...
//            }  
        ...
        //中国大陆
        if($_POST['region_type'] == 1){
            if (!$_POST['delivery']['ship_addr'] || !$_POST['delivery']['ship_name'])
            {
                if (!$_POST['delivery']['ship_addr'])
                {
                    $msg .= app::get('b2c')->_("收货地址不能为空！")."<br />";
        ...
        //日本酒店
        if($_POST['region_type'] == 2){
            if (!$_POST['hope_service_date'])
            {
                $msg .= app::get('b2c')->_("希望送达日不能为空！")."<br />";
                if (strpos($msg, '<br />') !== false)
                {
                    $msg = substr($msg, 0, strlen($msg) - 6);
                }
        ...
        //日本机场
        else if($_POST['region_type'] == 3){
            if(!$_POST['flight_no'] || !$_POST['departure_time']){
                if (!$_POST['flight_no'])
                {
                    $msg .= app::get('b2c')->_("航班号不能为空！")."<br />";
                }
                
                if (!$_POST['departure_time'])
                {
                    $msg .= app::get('b2c')->_("起飞时间不能为空！")."<br />";
                }
    ...
    */          $obj_order_create = kernel::single("b2c_order_create");
                ...
                $order_data['region_type'] = $_POST['region_type'];
                //收货人姓名
                $order_data['firstname'] = strtoupper($_POST['firstname']);
                $order_data['lastname'] = strtoupper($_POST['lastname']);
                //护照号码
                $order_data['passport'] = strtoupper($_POST['passport']);
                //日本酒店的场合
                if($order_data['region_type'] == 2){
                    //希望送达日
                    $order_data['hope_service_date'] = strtotime($_POST['hope_service_date']); 
                }
                //日本机场的场合
                else if($order_data['region_type'] == 3){
                    //航班号
                    $order_data['flight_no'] = $_POST['flight_no'];
                    //起飞时间
                    $order_data['departure_time'] = strtotime($_POST['departure_time'].' '.$_POST['_DTIME_']['H']['departure_time'].':'.$_POST['_DTIME_']['M']['departure_time']); 
                }
                ...
                if ($result)
                {
					...
					if($_POST['fastbuy']){
						...
						$this->cart_model->remove_object('', $Store_goods);
					}
