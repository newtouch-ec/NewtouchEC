...
class business_ctl_site_member extends b2c_ctl_site_member
{
    ...
    function _editor($cat_id,$type_id){
        ...
        $this->pagedata['goods_max_buy_num_per_order_switch'] = $this->app_b2c->getConf('goods.max_buy_num_per_order_switch');
        ...
        $this->pagedata['adjgoods_filter'] = array('goods_id|noequal'=>$this->goods_id,'marketable'=>'true','store_id'=>$this->store_id);
...
//        $this->pagedata['spec_default_pic'] = $this->app_b2c->getConf('spec.default.pic');
    ...
    public function doAddGrp($goodsid){
        ...
        //过滤下架商品,过滤商品本身（配件）@lujy
        //$this->pagedata['adjgoods_filter'] = array('goods_id|in'=>$goods);
        $this->pagedata['adjgoods_filter'] = array('goods_id|noequal'=>$goodsid,'marketable'=>'true','store_id'=>$this->store_id);
        $this->pagedata['store_id'] = $this->store_id;
        $this->pagedata['region_id'] = $this->region_id;
...
//                }
    ...
    function dlyaddress_delete($da_id, $mdl_name="business_mdl_dlyaddress")
    {
        $url = $this->gen_url(array('app'=>'business','ctl'=>'site_member','act'=>'dlyaddress'));
        if(empty($da_id)){
			echo json_encode(array('status'=>'failed','msg'=>'没有可以删除的数据'));exit;
            //$this->splash('failed',$url , app::get('b2c')->_('没有可以删除的数据'),'','',false);
        }
        ...
        if(method_exists($o, 'pre_recycle')){
            if(!$o->pre_recycle($rows)){
				echo json_encode(array('status'=>'failed','msg'=>app::get('b2c')->_('$o->recycle_msg')));exit;
                //$this->splash('failed',$url , app::get('b2c')->_('$o->recycle_msg'),'','',false);
            }
        }
        foreach($rows as $k=>$v){
...
//		$arrayorser = $order->db->select($sql);
		...
		$search_order=$this->search_order($order_id,$goods_name,$goods_bn,$sql);
		

		$arrayorser=$search_order;
		if(empty($arrayorser)){
			$msg='没有找到相应的订单！';
		}else{
			$aData = $order->fetchByShop($this->app->member_id,$store_id,$nPage-1,'','',$search_order);
		}
    ...
    * @return array
    */
    private function search_order($order_id,$goods_name,$goods_bn,$str_where='1=1'){
        //防止SQL注入
        ...
        $strsql .= ' and '.$str_where;
//        echo $strsql.' and '.$str_where;exit;
    ...
    */
    private function get_search_order_ids($type='',$time=''){
		...
		if($type == 'nopayed'){
            ...
            $type_sql=" pay_status='1' and ship_status <> '1' and ship_status <> '3' and ship_status <> '4' and status='active' ";
		}else if($type == 'finish'){
        ...
        $str_where = "store_id=".$store_id." AND ". $time_sql.' AND '.$type_sql;
        return $str_where;
    ...
    function index() {
        ...
        foreach($package_activityInfo as $key=>$value){
			...
			$package_activityInfo[$key]['activity_type'] = 'package';
        }
       //限时抢购
        ...
        foreach($timedbuy_activityInfo as $key=>$value){
			...
			$timedbuy_activityInfo[$key]['activity_type'] = 'timedbuy';
        }
		//团购
        $nowTime = time();
        $filter = array(
        ...
        foreach($groupbuy_activityInfo as $key=>$value){
			...
			$groupbuy_activityInfo[$key]['activity_type'] = 'groupbuy';
        }
		//积分换购
        ...
        foreach($scorebuy_activityInfo as $key=>$value){
			...
			$scorebuy_activityInfo[$key]['activity_type'] = 'scorebuy';
        }
     //秒杀活动
        ...
        foreach($spike_activityInfo as $key=>$value){
			...
			$spike_activityInfo[$key]['activity_type'] = 'spike';
        }
    ...
    function checkroles(){
        ...
        //非菜单项不做检查
        if(!in_array($permission,$menus)&&($action!="goods_add"&&$action!="goods_add_entity")){
            return;
        }
        ...
        if($sto->isshoper == 'true'){
          ...
          switch($data['approved']){
            ...
            //店铺违规处理
            //限制发布商品 limit_goods
            if($data['limit_goods']=='1' && ( $action=='goods_add'|| $action=='goods_type_select'|| $action=='goods_add_entity' || $action=='goods_import')){
            ...
            if (in_array($permission, $workground) or  $permission =='app=business&ctl=site_member&act=index' or in_array($action,$arrayexit)) {
                ...
                return;
                 ...
                 $this->splash('failed', 'back', app::get('b2c')->_('您没有相应的权限，请与店主联系！'));
            }
    ...
    */
	...
	function security($type = ''){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
         $this->path[] = array('title'=>app::get('b2c')->_('修改密码'),'link'=>'#');
          $GLOBALS['runtime']['path'] = $this->path;
        $obj_member = app::get('b2c')->model('members');
        $this->pagedata['mem'] = $obj_member->dump(app::get('b2c')->member_id,'*',array(':account@pam'=>array('*')));
    ...
    function save_security(){
       $url = $this->gen_url(array('app'=>'business','ctl'=>'site_member','act'=>'security'));
        $obj_member = app::get('b2c')->model('members');
        $result = $obj_member->save_security(app::get('b2c')->member_id,$_POST,$msg);
        if($result){
    ...
    function save_security_issue(){
       $url = $this->gen_url(array('app'=>'business','ctl'=>'site_member','act'=>'security'));
        $obj_member = app::get('b2c')->model('members');
       if($obj_member->save_security(app::get('b2c')->member_id,$_POST,$msg)){
           $this->splash('success',$url,$msg,'','',true);
       }
       else{
           $this->splash('failed',$url,$msg,'','',true);
    ...
    public function save_security_question(){
        $url = $this->gen_url(array('app'=>'business','ctl'=>'site_member','act'=>'security'));
        $obj_member = app::get('b2c')->model('members');
        $result=$obj_member->check_oldpasswd(app::get('b2c')->member_id,$_POST['old_passwd'],$msg);
        if(!$result){
    ...
    function do_verify_email(){
        ...
        if($return_token){
            ...
            $obj_member = app::get('b2c')->model('members');
            $mem=$obj_member->dump(app::get('b2c')->member_id);
            ...
            if($obj_member->is_exists_email($_POST['email'],app::get('b2c')->member_id)){
                $this->splash('failed','','该邮箱已经被使用，请更换邮箱。','','',true);
            }
            $_POST = $this->check_input($_POST);

            $_POST['member_id'] = app::get('b2c')->member_id;
            $_POST['status'] = 'n';
            ...
            $rows = app::get('pam')->model('account')->getList('*', array('account_type'=>'member', 'account_id'=>app::get('b2c')->member_id));

            $login_name = $rows[0]['login_name'];
            $turl = $this->gen_url(array('app'=>'business','ctl'=>"site_member",'act'=>"do_verify_email",'full'=>'1','arg0'=>$token));
           
            if($this->send_email($login_name, $_POST['email'], $turl,app::get('b2c')->member_id)){
    ...
    function send_email($login_name, $user_email, $url,$member_id){
        $ret = app::get('b2c')->getConf('messenger.actions.account-verifyEmail');
        $ret = explode(',', $ret);
        ...
        $data['shopname']=app::get('b2c')->getConf('system.shopname');
        $obj_account = app::get('b2c')->model('member_account');
        $result=$obj_account->fireEvent('verifyEmail', $data, $member_id);
        return $result;
    }
    ...
    function do_verify_mobile(){
        ...
        $this->begin(array('app'=>'business','ctl'=>'site_member','act'=>'security'));
        if($_POST['contact']['phone']['mobile'] && !preg_match('/^1[34578][0-9]{9}$/',$_POST['contact']['phone']['mobile'])){
            $this->splash('failed', '', app::get('b2c')->_('手机输入格式不正确'),'','',true);
        }
        $member_model = &$this->app->model('members');
        $result=$member_model->check_oldpasswd(app::get('b2c')->member_id,$_POST['old_passwd'],$msg);

        if(!$result){
            $this->splash('failed','',$msg,'','',true);
        }
        $mem=$member_model->dump(app::get('b2c')->member_id);
    ...
    function getMobileCode(){
        ...
        if($objmember->check_mobile($mobile, $message,app::get('b2c')->member_id)){
            //验证是否是恶意请求验证码
            $request_time = time();
    ...
    function goods_import_data(){
        ...
        if($msgList!==true){
            ...
            if($result){
                ...
                echo '<em style="color:#2f302a">导入成功。</em>';
            }else{
                $db->rollback();
                echo '<em style="color:#e32a2f;">商品编号【'.$errsdf['bn'].'】导入失败，请检查对应信息。</em>';
            }
            exit;
        }
    }
    ...
    function toAdd_entity(){
        ...
        foreach($goods['product'] as $k => $p){
            ...
            //优化提示信息
            if($goods['status'] != 'false' && intval($p['store']) == 0){ 
				if($goods['create'] == 'member'){
					$this->splash('failed',$url , app::get('b2c')->_('请确认已经手动输入卡密信息'),'','',true);
				}else{
					$this->splash('failed',$url , app::get('b2c')->_('请确认库存数量设置大于0'),'','',true);
				}
			}
            if (is_null( $p['store'] )){$goods['product'][$k]['freez'] = null;$goods['product'][$k]['store'] = null;}
            if(empty($p['bn'])) continue;
    ...
    function my_entity($nPage=1){
        ...
        $count = $obj_items->count(array('store_id'=>$this->store_id));
    ...
    public function settlement_list($page = 1)
    {
		...
		$allCount = $obj->count(array('store_id' => $store_id, 'disabled' => 'false'));
        $data = $obj->getList('*', array('store_id' => $store_id, 'disabled' => 'false'),
            ($page - 1) * $pageLimit, $pageLimit);

        $total = ceil($allCount / $pageLimit);
