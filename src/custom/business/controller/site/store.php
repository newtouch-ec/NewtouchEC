    ...
    */
    ...
    public function storeapplystep3() { 
        ...
        */
        ...
        //店铺提交开店申请时取得默认金额
        $shopinfo['order_max_money'] = app::get('b2c')->getConf('shops.max_money_per_order');
    ...
    public function storeapplystep4() {
        ...
        if (empty($_getParams)) {
            ...
            if ((!$accountid) or $accountid <> $this -> member['member_id']) {
                ...
                $this -> splash('failed', 'back', app :: get('business') -> _('提交失败，请去店铺管理中心继续设置。'), 'redirect');
                exit;
            } 
        } 
    ...
    public function storeapplyend() {
        ...
        if ($storemanger_model -> save($shopinfo)) {
			...
			if ($isedit) {
				$this -> splash('success', $url, app :: get('business') -> _('提交成功'), true, 1, true); 
			}else{
				echo json_encode(array('status' => 'success', 'msg' => app :: get('business') -> _('提交成功').$brandmsg,'url'=>$url));
    ...
    function storeroles() {
     ...
     function procHTML(&$tree) {
        ...
        if ($tree['label']) {
            $html .= "<li style='text-align:left;font-weight:bold;_font-style:italic;'>" . $tree['label'];
        } 
    ...
    function _checkData($aData) {
     ...
     function del_storecoupon() {
        ...
        $filter = array(
         ...
         $coupons_mdl = app::get('b2c')->model('coupons');
         $couponsArr = $coupons_mdl->getRow('*',$filter);
         $online_num = $couponsArr['online_num'];
         $coupon_status = $couponsArr['cpns_status'];
         //线上领取数大于0，优惠劵不能删除
         if($online_num>0 && $coupon_status==1){
             echo json_encode(array('status' => 'false', 'message' => '<span class="font-red">&nbsp;' . app :: get('b2c') -> _('使用中的优惠劵不能删除。') . '</span>'));
             return;
         }
