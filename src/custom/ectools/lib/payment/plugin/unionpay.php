...
final class ectools_payment_plugin_unionpay extends ectools_payment_app implements ectools_interface_payment_app {
    ...
    public function __construct($app){
        ...
        $this->submit_url = $this->app->res_url."/../../../custom/ectools/lib/payment/plugin/AuthorizeExec.php";
...
<a target="_blank" href="https://www.onlinepayment.com.my/MOLPay/">'.app::get('ectools')->_('如何使用支付宝支付？').'</a>';
 ...
 // echo('<pre>');print_r($mer_id.' '.$mer_key.' '.$subject.' '.$orderDetail);exit();
		...
		//金额
        $this->add_field('amount',number_format($payment['cur_money'],0,"",""));
        //returnurl
    ...
    public function callback(&$recv)
	{
		...
		*/
		...
		elseif($recv['mstatus'] =='pending')
		{
			...
			$arrPayment = $obj_payments->getRow('merge_payment_id',array('payment_id'=>$payment['bill_id']));
			
			if(empty($arrPayment['merge_payment_id'])){
				$ret['payment_id'] = $payment['bill_id'];
			}else{
				$ret['payment_id'] = $arrPayment['merge_payment_id'];
			}
            //$ret['payment_id'] = $payment['bill_id'];
