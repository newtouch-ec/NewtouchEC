...
class ectools_regions_operation
{
     ...
     * @reutrn boolean
     */
    public function updateRegionData_old() {
        $this->getAllRegions();
    ...
    public function updateRegionData() {
        if(ECAE_MODE){
            $this->app->setConf('system.region_Data',$this->createRegionJson('mainland','region_Data'));
            $this->app->setConf('system.hotel_Data',$this->createRegionJson('hotel','hotel_Data'));
            $this->app->setConf('system.airport_Data',$this->createRegionJson('airport','airport_Data'));
        }else{
            $str = "";
            $str .= $this->createRegionJson('mainland','region_Data');
            $str .= $this->createRegionJson('hotel','hotel_Data');
            $str .= $this->createRegionJson('airport','airport_Data');
            
            // 文件名字
            if(file_exists(CUSTOM_CORE_DIR.'/ectools/statics/js/region_data.js')){
                $filename = CUSTOM_CORE_DIR.'/ectools/statics/js/region_data.js';
            }elseif(file_exists(ROOT_DIR.'/app/ectools/statics/js/region_data.js')){
                $filename = ROOT_DIR.'/app/ectools/statics/js/region_data.js';
            }
            $handle = fopen($filename, "w");
            if(!$handle) {
                fclose($handle);
    ...
    private function createRegionJson($region_type,$region_name) {
        $this->getRegionsByType($region_type);
        $this->regions = (array)$this->regions;
        ...
        unset($this->regions);
        // 文件数据
        $str = "var ".$region_name."='".json_encode($regions)."';\r\n";
        $str .= $region_name." = JSON.decode(".$region_name.");\r\n";
        return $str;
    }
    
    private function getRegionsByType($region_type, $pkey='', &$region_count=array()) {
        $show_depth = $this->app->getConf('system.area_depth');
        $sql="select region_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1)." where p_region_id=r.region_id) as child_count from ".$this->model->table_name(1)." as r where r.package='".$region_type."' and r.p_region_id is null order by ordernum asc,region_id";
        $row = $this->model->db->select($sql);
