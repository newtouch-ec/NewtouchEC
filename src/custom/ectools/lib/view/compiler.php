...
class ectools_view_compiler{
    ...
    public function compile_modifier_cur_odr($attrs,&$compile) {
        ...
        if(!strpos($attrs,',') || false!==strpos($attrs,',')){
            ...
            //mod by Seven for BBC150630001-货币千分位符号 start
            ...
            if (count($arr_attributes) < 3)
            {
                $attrs .= ',false';
            }
            if (count($arr_attributes) < 4)
            {
                $attrs .= ',false';
            }
            if (count($arr_attributes) < 5)
            {
                $attrs .= ',app::get(\'b2c\')->getConf(\'system.money.decimals\')';
            }
            if (count($arr_attributes) < 6)
            {
                $attrs .= ',app::get(\'b2c\')->getConf(\'system.money.operation.carryset\')';
            }
            if (count($arr_attributes) < 7)
            {
                $attrs .= ',app::get(\'b2c\')->getConf(\'system.money.separator\')';
            }
            //mod by Seven for BBC150630001-货币千分位符号 end
