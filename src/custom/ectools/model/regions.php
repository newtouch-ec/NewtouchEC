...
class ectools_mdl_regions extends dbeav_model{
     ...
     * @return array region_path
     */
    public function getParentByLocalName($postCode='')
    {
        $sql = "SELECT A.region_path FROM ".$this->table_name(1)." AS A,sdb_ectools_postcode AS B WHERE B.post_office=A.local_name AND B.region_id=A.p_region_id AND B.postcode='".$postCode."'";
    ...
    public function getRegionByParentId($parentId)
    {
     ...
     /**
     * 得到指定p_region_id的地区信息
     * @params string p_region_id
     * @return array 信息数组
     */
    public function getRegionBypRegionId($p_region_id)
    {
        $sql="select region_id,local_name from ".$this->table_name(1)." where p_region_id=".intval($p_region_id);
        return $this->db->select($sql);
    }
    
    //根据ID取得详细信息[全字段]
    public function getRegionByRegionId($region_id)
    {
        $sql="select * from ".$this->table_name(1)." where region_id=".intval($region_id);
        return $this->db->select($sql);
    }
